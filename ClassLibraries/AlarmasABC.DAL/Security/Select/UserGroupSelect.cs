...
namespace AlarmasABC.DAL.Security.Select
{
    public class UserGroupSelect:DataAccessBase
    {
        ...
        public UserGroupSelect(string _schemeID,string _comID)
        {
            this.SchemeID = _schemeID;
            this.ComID = _comID;
           	Command = "SELECT * FROM tblGroup WHERE groupID IN (SELECT groupID FROM tblUser" +
					  " WHERE uID IN (SELECT userID FROM tblUserWiseScheme WHERE schemeID = :schemeID))" +
					  " AND comID = :comID AND coalesce(isDelete,'0') != '1' ORDER BY groupName ASC;";
        }
