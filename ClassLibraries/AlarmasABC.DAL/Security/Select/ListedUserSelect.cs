...
{
    public class ListedUserSelect:DataAccessBase
    {

        public ListedUserSelect(string _schemeID, string _comID)
        {
            this.SchemeID = _schemeID;
            this.ComID = _comID;
			Command = "SELECT uID,login FROM tblUser WHERE uID IN (SELECT userID" +
