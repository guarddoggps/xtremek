...
{
    public class SchemeUserListSelect:DataAccessBase
    {

		public SchemeUserListSelect()
       	{
			Command = "SELECT uID,login FROM tblUser WHERE comID = :comID" +
       ...
       public void LoadUserList(IList<User> _GLists)
       {
           ...
           try
           {
               ...
               while (_dr.Read())
               {
                   _GLists.Add(new User(int.Parse(_dr[0].ToString()), _dr[1].ToString()));
               }
           }
           catch (Exception ex)
           {
