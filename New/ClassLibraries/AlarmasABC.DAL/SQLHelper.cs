// ===============================================================================
    // this File directly comunicate with Database. not other file will connect with database.
// ==============================================================================
...
namespace Microsoft.ApplicationBlocks.Data
{
    ...
    public sealed class SqlHelper
    {
		...
		public static DataTable ExecuteDataTable(String connectionString, CommandType commandType, string commandText, NpgsqlParameter [] parameters)
        {
            if( connectionString == null || connectionString.Length == 0 ) throw new ArgumentNullException( "connectionString" );
            ...
            {
				// Call the overload that takes a connection in place of the connection string
                return ExecuteDataTable(connection, commandType, commandText, parameters);
            }
        }
        ...
        public static DataTable ExecuteDataTable(NpgsqlConnection connection, bool isProc, string commandText)
        {
            // Pass through the call providing null for the set of NpgsqlParameters
            if(isProc)
                return ExecuteDataTable(connection, CommandType.StoredProcedure, commandText, (NpgsqlParameter[])null);
            else
            {
                return ExecuteDataTable(connection, CommandType.Text, commandText, (NpgsqlParameter[])null);
            }
        }
		...
		public static DataSet ExecuteDataset(NpgsqlConnection connection, CommandType commandType, string commandText, params NpgsqlParameter[] commandParameters)
		{
			...
			if (commandType == CommandType.Text && commandTexts.Length > 1) {
				i = commandTexts.Length - 1;
			}
			
			PrepareCommand(cmd, connection, (NpgsqlTransaction)null, commandType, commandTexts[0], commandParameters);
			...
			for (j = 1; i > 0; i--, j++) {
				cmd.CommandText = commandTexts[j];
				
				DataSet tmpDs = new DataSet();
				// Fill the DataSet using default values for DataTable names, etc
				da.Fill(tmpDs);
					
				DataTable dt = tmpDs.Tables[0].Copy();
				dt.TableName = "Table" + Convert.ToString(j);
        ...
        {	
			...
			{
				throw new Exception (" SqlHelper::ExecuteReader:: " + ex.Message);
			}
			finally
			{
				connection.Close();
